...
if os.path.exists("C:\\AOBREGON\\"+nombre_archivo):
    libro = openpyxl.load_workbook("C:\\AOBREGON\\"+nombre_archivo)   #El archivo ya existe
else:
    libro = openpyxl.Workbook()                                       #El archivo no Existe


# Consulta para obtener las incidencias
consulta = "SELECT Fecha, MIN(Hora) as HoraEntrada, MAX(Hora) as HoraSalida, Alias FROM newschema.fichajes1 JOIN newschema.empleados1 ON fichajes1.IdEmpleado = empleados1.Codigo WHERE  Fecha = '20230329' GROUP BY IdEmpleado HAVING COUNT(*) = 1 ORDER BY IdEmpleado ASC"
